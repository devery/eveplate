...
@Component({
    ...
    template: `
        <div class="Explorer">
            ...
            <h3>User Account:</h3>
            <span *ngIf="!account; else elseBlock">{{DEFAULT_MESSAGE}}</span>
...
export class AppComponent implements AfterViewInit {
    DEFAULT_MESSAGE = 'Please sign in to MetaMask';
    account: string = '';
    appAddr: any = null;
    ...
    async addProduct(data) {
        try {
            await devery.addProduct(this.account, data, 'batch 001', new Date().toISOString(), 'Unknown place')
        } catch (e) {
            if (e.message.indexOf('User denied')) {
                console.log('The user denied the transaction')
            }
